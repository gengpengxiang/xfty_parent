...
public class GaijinDetailActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        // set Adatper
        mAdapter = new LatestNewsHomeAdapter2(mDataList, getString(R.string.text_desc_empty_news_gaijin));
        mAdapter.setOnMyItemClickListener(new LatestNewsHomeAdapter2.OnMyItemClickListener() {
            @Override
...
//                mXRefreshView.setPullLoadEnable(true);
    ...
    private class MyClassCommendTypeAsyncTask extends AsyncTask<String, Integer, List<SubjectInfo>> {
        ...
        protected List<SubjectInfo> doInBackground(String... params) {
            ...
            try {
                //1是点赞，2是待改进
                newList = mService.getClassCommendTypeFromAPI(kidId,"2");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private class MyGetStudentAllNewsTask extends AsyncTask<String, Integer, List<ClassNewsInfo>> {
        ...
        protected List<ClassNewsInfo> doInBackground(String... params) {
            ...
            try {
                ...
                dataList = mService.getClassCommendNewsFromAPI(studentID, reasonTypeID, "2",pageIndex);
            } catch (Exception e) {
                e.printStackTrace();
                LL.e(e);
                dataList = null;
        ...
        protected void onPostExecute(String[] result) {
            if (!StringUtils.isEmpty(result[0]) && result[0].equals("1")) {
                ...
                if (!StringUtils.isEmpty(result[0]) && result[0].equals("4")) {
                    T.showShort(GaijinDetailActivity.this, result[1]);
                } else {
