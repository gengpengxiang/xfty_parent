...
import com.bj.hmxxparents.widget.LegendGridLayoutManager;
...
public class StudentReportFragment3 extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_report_3, container, false);
    ...
    private void initView() {
        ...
        int padding = DensityUtils.dp2px(getActivity(), 6);
        int itemPadding = DensityUtils.dp2px(getActivity(), 6);
        // 下拉刷新控件
        mRecyclerView.setHasFixedSize(false);
        mRecyclerView.setPadding(padding, 0, padding, 0);
        LegendGridLayoutManager layoutManager = new LegendGridLayoutManager(getActivity(), 3, 4);
    ...
    public void onMessageEvent(ReportInfo event) {
        LL.i("StudentReportFragment3 - 收到消息 : 学生点赞数" + event.userCommendCount);
        tvBadgeNumber.setText(event.userCommendCount);
        updateData(event.userCommendPieMap);
    }

    private void updateData(HashMap<String, Integer> userCommendPieMap) {
        if (userCommendPieMap == null) return;
        mDataList.clear();
        for (Map.Entry<String, Integer> entry : userCommendPieMap.entrySet()) {
    ...
    protected void onVisible() {
        super.onVisible();
        MobclickAgent.onPageStart("report3");
        if (!isFirstInit) {
            mChart.animateY(1800, Easing.EasingOption.EaseInOutQuad);
            setData(mDataList);
        }
    ...
    protected void onInVisible() {
        super.onInVisible();
        MobclickAgent.onPageEnd("report3");
    }
